this.pov_autopsy_hexe2_blueprint <- this.inherit("scripts/crafting/blueprint", {
	...
	function create()
	{
		...
		this.m.PreviewCraftable = this.new("scripts/items/autopsy/pov_autopsy_hexe2_item");
		this.m.Cost = 110;
		local ingredients = [
			{
	...
	function onCraft( _stash )
	{
		if (this.Math.rand(1, 100) <= 15)
			{
				_stash.add(this.new("scripts/items/misc/anatomist/pov_hexe_mutagen_upgrade_item"));
			}
